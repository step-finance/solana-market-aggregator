...
describe("xSTEP source", () => {
  it("Loads price relative to STEP", async () => {
    ...
    const staticXStepMarketData = (
      await xStepSource.query(staticStepMarketData)
    )[XSTEP_MINT];
    const xStepMarketData = (await xStepSource.query(mockedStepMarketData))[
      XSTEP_MINT
    ];
    expect(
