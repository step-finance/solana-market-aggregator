...
export const isSaberLiquidityNetwork = (network: Network): network is SaberLiquidityNetwork =>
  network === "mainnet-beta" || network === "devnet";

export const getSaberTokenInfos = async (cluster: Cluster): Promise<TokenInfo[]> => {
  if (isSaberLiquidityNetwork(cluster)) {
    const formattedNetwork = formatNetwork(cluster);
    const rawSaberTokenList = (
      await axios.get<{ tokens: TokenInfo[] }>(`https://registry.saber.so/data/token-list.${formattedNetwork}.json`)
    ).data.tokens;
